...
class PowerShellSession:
    ...
    def __init__(self):
        ...
        # Get the current user's module path
        current_module_path = os.path.expanduser("~\\Documents\\WindowsPowerShell\\Modules")
        
        # Get the system module paths from the current process
        system_module_path = os.environ.get('PSModulePath', '')
        
        # Combine all module paths
        module_path = f"{current_module_path};{system_module_path}"
        
        # Create the environment with the combined module paths
        env = os.environ.copy()
        env["PSModulePath"] = module_path

        # Initialize PowerShell with the current user's environment
        ...
        # Set the execution policy to allow module loading
        self._execute_command("Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force")
        
        # Import the required modules
        self._execute_command("Import-Module Microsoft.Graph.Authentication -Force -ErrorAction SilentlyContinue")
        self._execute_command("Import-Module MicrosoftTeams -Force -ErrorAction SilentlyContinue")
        self._execute_command("Import-Module ExchangeOnlineManagement -Force -ErrorAction SilentlyContinue")
        self._execute_command("Import-Module Microsoft.PowerShell.SecretManagement -Force -ErrorAction SilentlyContinue")

    def _execute_command(self, command: str) -> dict:
        """
        Execute a PowerShell command and return the result as a dictionary.
        """
        try:
    ...
    def execute(self, command: str) -> dict:
        """
        Execute a PowerShell command and return the result as a dictionary.
        This is a wrapper around _execute_command that handles retries.
        """
        max_retries = 3
