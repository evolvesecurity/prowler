...
class entra_conditional_access_grant_controls_check(Check):
    def execute(self) -> List[CheckReportM365]:
        ...
        if not policies:
            ...
            for policy in policies:
                ...
                # Check if any value in AuthenticationStrength is not None or empty
                is_configured = False
                if auth_strength and isinstance(auth_strength, dict):
                    for v in auth_strength.values():
                        if v not in (None, [], {}, ""):
                            is_configured = True
                            break
                if is_configured:
                    found_auth_strength = True
                    configured_policies.append(display_name)
            if found_auth_strength:
                status = "PASS"
                status_extended = f"At least one enabled Conditional Access policy has authentication strength configured in grantControls: {configured_policies}."
            else:
                status = "FAIL"
                status_extended = "No enabled Conditional Access policies have authentication strength configured in grantControls."
